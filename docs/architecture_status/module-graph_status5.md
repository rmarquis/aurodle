...
> Reflects the codebase as of 2026-05 after the Issues C–F refactors.
> Supersedes `module-graph_status4.md`.
> Sentrux scan: 2026-05-03, quality signal 6527/10000.
>
...
flowchart TD
    ...
    %% ── display: plan rendering (imports solver for BuildPlan types) ─────
    display --> context & solver & devel & pacman
...
The longest file-to-file dependency path, down from 14 in status4:
...
s_conflicts level  7   (Layer 3)  ← s_graph now at depth 3; conflicts takes registry branch
solver      level  8   (Layer 3)
display     level  9   (Layer 4)  ← new module; imports solver for BuildPlan types
analysis    level 10   (Layer 4)  ← imports display
commands    level 11   (Layer 4 hub)
main        level 12   (Layer 5)
```

Two levels were saved since status4:
- Issue D removed `s_graph → registry`; s_graph dropped from 7 → 3.
- Issue C moved `displayPlan`/`displayInstallList` out of `context.zig`; `context` dropped from 10 → 7.

## What changed since status4

| Change | Details |
|---|---|
| **New** `source.zig` (Layer 0) | Holds the `Source` enum; `registry.zig` re-exports it; `solver/graph.zig` now imports `source.zig` directly instead of `registry.zig` |
| **New** `commands/display.zig` (Layer 4) | All plan rendering extracted from `context.zig`; imports `solver.zig` for `BuildPlan`/`BuildEntry` types |
| **New** `commands/outdated.zig` (Layer 4) | `collectOutdated` + `checkDevelPackages` extracted from `query.zig`; shared by `query` and `build_cmd` |
| `s_graph` edges | Was `s_graph → registry & aur`; now `s_graph → source & aur` (depth 7 → 3) |
| `registry` edges | Added `registry → source` |
| `context` edges | Removed `→ solver`, `→ devel`, `→ git`; depth 10 → 7 |
| `query` edges | Removed `→ git` (moved to `outdated.zig`); added `→ outdated` (re-exports `collectOutdated`, `checkDevelPackages`) |
| `build_cmd` edges | Removed `→ query`; added `→ display`, `→ outdated` |
| `analysis` edges | Added `→ display` |
| `commands` hub edges | Re-exports `displayPlan`/`displayInstallList` from `display` instead of `context` |

## Remaining issues

| Issue | Edge | Type | Expected gain |
|---|---|---|---|
| G | `display → solver` | Depth floor | Depth 12 → 11 if `BuildPlan`/`BuildEntry` extracted to `plan.zig` |
| H | 93/117 cross-module edges (79%) | Modularity plateau | No obvious low-cost structural win; reflects domain coupling |
